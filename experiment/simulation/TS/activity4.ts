...
function verify_act4() {
	...
	pp.showdescription(
		`<p class='discription_text'>All entered values are correct, click on next to see data table.</p>`,
		3
	);

	pp.addtorightpannel(
		`<button id='panel1_btn' class='btn btn-primary' onclick='load_table();' style="bottom:12.5%">Next</button>`,
		3
	);
...
function act4_verify2() {
	let val1: HTMLInputElement = <HTMLInputElement>(
		document.getElementById(`mean-d`)
	);
	let val2: HTMLInputElement = <HTMLInputElement>(
		document.getElementById(`velocity-inp`)
	);
	...
	// activity5();
	pp.showdescription(
		`<p class='discription_text'>Successfully Verified!!</p>`,
		3
	);

	pp.addtorightpannel(
		`<button id='panel1_btn' class='btn btn-primary' onclick='activity5();' style="bottom:12.5%">Next</button>`,
		3
	);
