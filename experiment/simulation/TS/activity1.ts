...
function activity1() {
	...
	pp.showtitle(
		`<p id="exp-title">Ultrasonic Waves Velocity in Liquid Media</span><p>`,
		3
	);
	pp.showdescription(
		`<p class='discription_text'>Let's identify the components required</p>`,
		3
	);
	pp.showscore(0, 3);
	canvas = pp.canvas;
...
function a1_load_questions() {}
{
	...
	question.push({
		srno: 1,
		question:
			"Select <span style='color: #018fc3'>Radio Frequency Generator</span>",
		ans: 'generator',
		hint: ['Tower like component'],
	});
	question.push({
		srno: 2,
...
function load_higlighted_images() {
	...
	a1_labels = [
		new Chemistry.Text(
			...
			new Chemistry.Point(1280, 80),
			canvas
		),

		new Chemistry.Text(
			'Interferometer',
			new Chemistry.Point(320, 230),
			canvas
		),
	];
