namespace Chemistry {
	...
	export class Circle extends Geometry {
		...
		draw_connection() {
			...
			this.context.lineTo(
				this.objconnected.stpt.x * lscale,
				this.objconnected.stpt.y * lscale
			);
			this.context.stroke();
	...
	export class L_bracket extends Geometry {
		...
		protected calculate(): void {
			...
			this.points.push(
				new Point(
					(this.stpt.x - 20) * lscale,
					(this.stpt.y - 20) * lscale
				)
			);
			this.points.push(
				new Point(
					(this.stpt.x - 20) * lscale,
					(this.stpt.y - 20 + 100) * lscale
				)
			);
			this.points.push(
				new Point(
					(this.stpt.x - 20 + 40) * lscale,
					(this.stpt.y - 20 + 100) * lscale
				)
			);
			this.points.push(
				new Point(
					(this.stpt.x - 20 + 40) * lscale,
					(this.stpt.y - 20 + 40) * lscale
				)
			);
			this.points.push(
				new Point(
					(this.stpt.x - 20 + 100) * lscale,
					(this.stpt.y - 20 + 40) * lscale
				)
			);
			this.points.push(
	...
	export class Sine extends Geometry {
		...
		draw() {
			...
			this.context.beginPath();
			this.context.moveTo(
				this.points[0].x * lscale,
				this.points[0].y * lscale
			);
			for (let i = 1; i <= 360; i++) {
				this.context.lineTo(
					this.points[i].x * lscale,
					this.points[i].y * lscale
				);
			}
			...
			this.context.stroke();
			this.context.strokeStyle = 'black';
			this.geo.stpt = new Point(
				this.points[this.k].x,
				this.points[this.k].y
			);
			this.geo.draw();
			this.update();
		}
	...
	export class anim_image extends Custome_image {
		...
		draw() {
			...
			this.context.drawImage(
				...
				this.dx - this.width,
				this.dy,
				(-this.dx / 2 + this.startx) * lscale,
				(this.dy / 2 - this.l) * lscale,
				(this.dx - this.width) * lscale,
				this.dy * lscale
			);
	...
	export class Virtual_circle extends Circle {
		constructor(stpt: Point, radius: number, canvas: HTMLCanvasElement) {
