...
let right_btn: Chemistry.Polygon;
let left_btn: Chemistry.Polygon;
let range = 0;
var arw: Chemistry.Custome_image;
var scale_1: Chemistry.Custome_image;
var scale_2: Chemistry.Custome_image;
var small_scale_1: Chemistry.Custome_image;
var small_scale_2: Chemistry.Custome_image;
var main_scale: Chemistry.Custome_image;
var vernier_scale: Chemistry.Custome_image;
var small_vernier_scale: Chemistry.Custome_image;
var left_cir: Chemistry.Virtual_circle;
var right_cir: Chemistry.Virtual_circle;
var click = 0;
let clicks_ar: Number[] = [];
let check_cir: Chemistry.Circle;
...
function activity3() {
	...
	pp.showtitle(
		`<p id="exp-title">Determination of ultrasonic waves velocity in liquid media<p>`,
		3
	);
	...
	canvas1 = <HTMLCanvasElement>document.getElementById('mycanvas1');
	canvas2 = <HTMLCanvasElement>document.getElementById('mycanvas2');
	canvas3 = <HTMLCanvasElement>document.getElementById('mycanvas3');
	canvas4 = <HTMLCanvasElement>document.getElementById('mycanvas4');
	...
	right_btn = new Chemistry.Polygon(
		new Chemistry.Point(1060, 510),
		20,
		3,
		canvas1
	);
	...
	left_btn = new Chemistry.Polygon(
		new Chemistry.Point(943, 510),
		20,
		3,
		canvas1
	);
	...
	left_cir = new Chemistry.Virtual_circle(
		new Chemistry.Point(943, 510),
		20,
		canvas1
	);
	...
	right_cir = new Chemistry.Virtual_circle(
		new Chemistry.Point(1060, 510),
		20,
		canvas1
	);
...
function load_images() {
	...
	arw = new Chemistry.Custome_image(
		arrow,
		new Chemistry.Point(470, 216),
		45,
		50,
		canvas1
	);
	...
	small_vernier_scale = new Chemistry.Custome_image(
		screw,
		new Chemistry.Point(1002, 574),
		43,
		171,
		canvas1
	);
	...
	small_scale_1 = new Chemistry.Custome_image(
		scale_hd,
		new Chemistry.Point(-36, 15),
		396 * 0.25,
		108 * 0.25,
		canvas2
	);
	...
	small_scale_2 = new Chemistry.Custome_image(
		scale_hd,
		new Chemistry.Point(60, 15),
		396 * 0.25,
		108 * 0.25,
		canvas2
	);
	...
	main_scale = new Chemistry.Custome_image(
		scale_main,
		new Chemistry.Point(95, 140),
		219 * 0.75,
		795 * 0.75,
		canvas3
	);
	...
	vernier_scale = new Chemistry.Custome_image(
		scale_vernier,
		new Chemistry.Point(93, 300),
		171 * 0.71,
		681 * 0.71,
		canvas3
	);
	...
	scale_1 = new Chemistry.Custome_image(
		scale_hd,
		new Chemistry.Point(-134.5, 45),
		396 * 0.8,
		108 * 0.8,
		canvas4
	);
	...
	scale_2 = new Chemistry.Custome_image(
		scale_hd,
		new Chemistry.Point(169.5, 45),
		396 * 0.8,
		108 * 0.8,
		canvas4
	);
	...
	var line = new Chemistry.Rectangle(
		0,
		50,
		new Chemistry.Point(75, 5),
		canvas3
	);
	scene1.add(line);

	check_cir = new Chemistry.Circle(
		new Chemistry.Point(1550, 500),
		20,
		canvas1
	);
	check_cir.color = 'red';
	...
	var txt = new Chemistry.Geo_Text(
		'Indicator',
		new Chemistry.Point(1580, 485),
		canvas1
	);
	txt.font = '40 Arial';
	scene1.add(txt);
...
function load_fluid() {
	let fluid_sel: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('fluid-dd')
	);
...
function set_fluid() {
	...
	let freq_sel: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('freq-dd')
	);
	...
	if (fluid_sel.value) {
		...
		pp.showdescription(
			`<p class='discription_text'>Select Frequency</p>`,
			3
		);
		show_panel(3);
	} else {
...
function set_frequency() {
	let freq: HTMLSelectElement = <HTMLSelectElement>(
		document.getElementById('freq-dd')
	);
	...
	pp.showdescription(
		`<p class='discription_text'>Take main scale and vernier scale reading.</p>

   <br>

   <p class='discription_text'>Note the reading when indicator truns green.</p>
   <br>
   <p class='discription_text'>Use left and right button beside RF Generator to rotate vernier scale. Click & hold the button for fast rotation.</p>`,
		3
	);
	show_panel(3);
...
//for button hold
function left_btn_down(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas1.height - (e.clientY - rect.y)) / lscale);
	let fluid = <HTMLSelectElement>document.getElementById('fluid-dd');
	...
	if (left_cir.isinside(new Chemistry.Point(x, y))) {
		if (fluid.value != '' && selected_frequency > 0) {
			...
			left_hold_id = setInterval(() => {
				...
				small_vernier_scale.stpt = new Chemistry.Point(
					small_x_ver,
					small_y_ver
				);
...
function left_touch_start(e: TouchEvent) {
	let x = Math.round((e.touches[0].clientX - rect.x) / lscale);
	let y = Math.round(
		(canvas1.height - (e.touches[0].clientY - rect.y)) / lscale
	);
	let fluid = <HTMLSelectElement>document.getElementById('fluid-dd');
...
function right_btn_down(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas1.height - (e.clientY - rect.y)) / lscale);
	let fluid = <HTMLSelectElement>document.getElementById('fluid-dd');
...
function right_touch_start(e: TouchEvent) {
	let x = Math.round((e.touches[0].clientX - rect.x) / lscale);
	let y = Math.round(
		(canvas1.height - (e.touches[0].clientY - rect.y)) / lscale
	);
